...
gridfs_compare_collections (mongoc_database_t *db)
{
   ...
   while (mongoc_cursor_next (expected_cursor, &expected_doc)) {
      ...
      // ASSERT (bson_compare (expected_doc, actual_doc) == 0);
                                ...
                                bson_t *assert)
{
   ...
   stream = mongoc_gridfs_bucket_open_download_stream (bucket, value, NULL);

   if (strcmp (err, "FileNotFound") == 0) {
      ASSERT (!stream);
      bson_destroy (arguments);
      return;
   }

   ret = mongoc_stream_read (stream, buf, 1000, 0, 0);

   if (strcmp (err, "ChunkIsMissing") == 0 ||
       strcmp (err, "ChunkIsWrongSize") == 0) {
      ASSERT (ret < 0);
      bson_destroy (arguments);
                      ...
                      bson_t *tests)
{
   ...
   ASSERT (bson_iter_init_find (&iter, tests, "act"));
   act = bson_new_from_data (bson_iter_value (&iter)->value.v_doc.data,
                             bson_iter_value (&iter)->value.v_doc.data_len);

   ASSERT (bson_iter_init_find (&iter, tests, "assert"));
   assert = bson_new_from_data (bson_iter_value (&iter)->value.v_doc.data,
                                bson_iter_value (&iter)->value.v_doc.data_len);

   if (bson_iter_init_find (&iter, tests, "arrange")) {
      arrange =
         bson_new_from_data (bson_iter_value (&iter)->value.v_doc.data,
                             bson_iter_value (&iter)->value.v_doc.data_len);
