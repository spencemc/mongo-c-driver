...
main (int argc, char *argv[])
{
   ...
   res = mongoc_gridfs_upload_from_stream (
      bucket, "example.txt", file_stream, NULL, &file_id, &error);
   if (!res) {
   ...
   res = mongoc_gridfs_open_download_stream (
      bucket, file_id, &download_stream, &error);
   if (!res) {
   ...
   while (bytes_read >= min_bytes) {
      printf ("%s", buf);
      bytes_read =
         mongoc_stream_read (&download_stream, &buf, 256, min_bytes, 0);
   }
   if (bytes_read == -1) {
   ...
   file_stream2 =
      mongoc_stream_file_new_for_path ("example-copy.txt", O_RDWR | O_CREAT, 0);
   res =
      mongoc_gridfs_download_to_stream (bucket, &file_id, file_stream2, &error);
   if (!res) {
