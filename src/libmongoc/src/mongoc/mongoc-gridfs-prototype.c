...
#include "mongoc-gridfs-prototype-upload-stream.h"
#include "mongoc-gridfs-prototype-bucket-file.h"

mongoc_gridfs_bucket_t*
mongoc_gridfs_bucket_new (mongoc_database_t *db,bson_t* opts /* create opts */, bson_error_t *error){
    ...
    bucket->chunks = mongoc_database_get_collection(db, strcat(prefix, ".files"));

    // check for errors

    bucket->files = mongoc_database_get_collection(db, strcat(prefix, ".chunks"));
...
mongoc_gridfs_open_upload_stream(mongoc_gridfs_bucket_t* bucket,
    ...
    return mongoc_upload_stream_gridfs_new(bucket_file);
}
...
mongoc_gridfs_upload_from_stream(mongoc_gridfs_bucket_t* bucket,
    ...
    //  2. continuously write to the upload stream from the given "source" stream.
