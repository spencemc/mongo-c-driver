...
_mongoc_upload_stream_gridfs_destroy (mongoc_stream_t *stream)
{
   ...
   mongoc_gridfs_bucket_file_destroy (gridfs->file);
   bson_free (stream);
...
_mongoc_upload_stream_gridfs_close (mongoc_stream_t *stream)
{
   ...
   /* QUESTION: Should mongoc_gridfs_bucket_file_save return an int? */

   mongoc_gridfs_bucket_file_save (gridfs->file);
                                     ...
                                     int32_t timeout_msec)
{
   ...
   ret = mongoc_gridfs_bucket_file_writev (gridfs->file, iov, iovcnt, 0);
